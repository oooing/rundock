...
Windows 正式版启动约 8 秒后自动检查更新，有新版会在后台下载。侧栏显示下载进度，完成后显示「新版已就绪，点击升级」。点击提示打开「设置 → 应用更新」，查看版本和可用的更新说明，再确认安装。每次启动自动检查一次，无更新或网络失败不会弹窗打断；也可在设置中手动检查、重试下载。关闭设置窗口不会中断下载。Web 版和开发版不自动检查或下载。
