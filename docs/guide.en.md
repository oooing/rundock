...
Windows production builds check for updates about 8 seconds after launch, show an update dialog when a new version is found, and download it in the background. Choose **Later**, close the dialog, or press Esc to dismiss it without interrupting the download. Completion does not reopen a dismissed dialog. A compact row in Settings keeps the current and available versions visible, with **View update / Continue installation**; the sidebar prompt also reopens the dialog. Installation always requires an explicit **Quit and install** click. The automatic check runs once per launch; no-update and network-failure results do not interrupt your work. Manual checks and download retries remain available. Web and development builds do not check or download automatically.

Language, motion effects, and runtime settings save automatically without a Save button. Runtime settings are under **Advanced settings**, collapsed by default: the stop timeout allows a clean exit, and the startup timeout allows services to become ready. Changes apply to the next start or stop without restarting RunDock. Health-check interval and log-retention controls have been removed because they were not connected to runtime behavior. Invalid values and save failures show inline messages with a retry action when appropriate.
